...
void KVCache::attention_kvhead_(const uint16_t *q_in_data, ggml_fp16_t *output,
  ...
  backend->do_work_stealing_job(
      ...
      [&](int task_id) {
        ...
        int block_idx =
            block_table_after_retrieval_kvhead_[batch_id][block_id][head_id];
        if (cache_seqlens_[batch_id] / config_.block_len == block_id) {
          ...
          if (config_.kv_type == ggml_type::GGML_TYPE_F16) {
            attn_with_kvcache_one_block_(
                ...
                GGML_TYPE_F16,
                (void *)&q_in_data[batch_id * config_.kv_head_num * n_gqa_ *
                                       config_.head_dim +
                                   head_id * n_gqa_ * config_.head_dim],
                seq_len_, 0, false, thread_local_attn_mask_[thread_id].data(),
                GGML_TYPE_F16, 0,
                k_cache_fp16_[layer_id_][head_id][block_idx].data(), 0, nullptr,
                nullptr, GGML_TYPE_F16, 1,
                v_cache_fp16_[layer_id_][head_id][block_idx].data(), 0, nullptr,
                ...
                thread_local_draft_[thread_id].data(), nullptr, cos_.data(),
                sin_.data());
          } else if (config_.kv_type == ggml_type::GGML_TYPE_Q4_0) {
            attn_with_kvcache_one_block_(
                config_.head_dim, config_.q_head_num / config_.kv_head_num,
                GGML_TYPE_Q8_0, q_q8_0_[batch_id][head_id].data(), seq_len_, 0,
                false, thread_local_attn_mask_[thread_id].data(),
                GGML_TYPE_Q4_0, 0,
                k_cache_q4[layer_id_][head_id][block_idx].data(), 0, nullptr,
                nullptr, GGML_TYPE_Q4_0, 1,
                v_cache_q4[layer_id_][head_id][block_idx].data(), 0, nullptr,
                nullptr, thread_local_attn_score_[thread_id].data(),
            ...
            attn_with_kvcache_one_block_(
                ...
                false, thread_local_attn_mask_[thread_id].data(),
                GGML_TYPE_Q8_0, 0,
                k_cache_q8[layer_id_][head_id][block_idx].data(), 0, nullptr,
                nullptr, GGML_TYPE_Q8_0, 1,
                v_cache_q8[layer_id_][head_id][block_idx].data(), 0, nullptr,
                nullptr, thread_local_attn_score_[thread_id].data(),
                thread_local_output_q8_0_[thread_id].data(),
                thread_local_attn_lse_[thread_id].data(),
                thread_local_draft_[thread_id].data(), nullptr, cos_.data(),
        ...
        if (batch_id == cur_batch_idx && head_id == cur_head_id) {
          ...
          for (int i = 0; i < n_gqa_; i++) {
            ...
            for (int j = 0; j < config_.head_dim; j++) {
              thread_local_cur_output_fp32_[thread_id][i * config_.head_dim +
                                                       j] =
                  thread_local_output_fp32_[thread_id]
                                           [i * config_.head_dim + j];
            }
          ...
          for (int i = 0; i < n_gqa_; i++) {
            ...
            if (std::abs(attn_lse_[cur_batch_idx][cur_head_id][i]) < 1e-6) {
              attn_lse_[cur_batch_idx][cur_head_id][i] =
                  thread_local_cur_attn_lse_[thread_id][i];
              for (int j = 0; j < config_.head_dim; j++) {
                output_fp32_[cur_batch_idx][cur_head_id][i * config_.head_dim +
                                                         j] =
                    thread_local_cur_output_fp32_[thread_id]
                                                 [i * config_.head_dim + j];
              }
  ...
  for (int batch_idx = 0; batch_idx < batch_size; batch_idx++) {
    for (int i = 0; i < config_.kv_head_num; i++) {
      ...
      for (int j = 0; j < n_gqa_; j++) {
        attn_lse_data[batch_idx * config_.kv_head_num * n_gqa_ + i * n_gqa_ +
                      j] = attn_lse_[batch_idx][i][j];
      }
    }
  }
...
void KVCache::attn(const ggml_fp16_t *q_in, ggml_fp16_t *output,
  ...
  if (config_.retrieval_type == RetrievalType::LAYER) {
    attn_initialize_layer_(batch_size, layer_idx, block_table, max_block_num,
                           cache_seqlens);
...
void KVCache::attn_with_kvcache(
  ...
  attn(q_in, output, attn_lse, layer_idx, generate_token_idx, q_len, batch_size,
       max_block_num, block_table, cache_seqlens, topk, init_block_num, local,
       backend);
      ...
      for (int i = 0; i < config_.kv_head_num; i++) {
        for (int j = 0; j < n_gqa_ * config_.head_dim; j++) {
          q_fp32_[batch_idx][i][j] =
              GGML_FP16_TO_FP32(q_in_data[batch_idx * config_.kv_head_num *
                                              n_gqa_ * config_.head_dim +
                                          i * n_gqa_ * config_.head_dim + j]);
        }
      }
...
void KVCache::calculate_block_similarity_layer_(
  ...
  if (batch_size == 1 &&
    ...
    for (int batch_id = 0; batch_id < batch_size; batch_id++) {
      ...
      int block_num =
          (seq_len / config_.block_len) - local_block_num - init_block_num;
      if (block_num <= 0) {
        continue;
      }
      bool is_seq = true;
        ...
        backend->do_work_stealing_job(
            ...
            [&](int task_id) {
              ...
              bool ok = llamafile_sgemm(
                  ...
                  config_.q_head_num * config_.head_dim,
                  block_similar_[batch_id].data() + init_block_num, block_num,
                  ith, nth, GGML_TASK_TYPE_COMPUTE, GGML_TYPE_F16,
            ...
            for (int i = 0; i < config_.head_dim; i++) {
              ...
              for (int q_id = 0; q_id < q_len; q_id++) {
                q_i += GGML_FP16_TO_FP32(
                    q_in_data[batch_id * q_len * config_.q_head_num *
                                  config_.head_dim +
                              q_id * config_.q_head_num * config_.head_dim +
              ...
              for (int anchor_id = 0; anchor_id < config_.anchor_num;
                ...
                qa_i = std::max(
                    qa_i,
                    GGML_FP16_TO_FP32(
                        anchor_[(long long)layer_idx * config_.max_block_num *
                                    config_.anchor_num * config_.q_head_num *
                                    config_.head_dim +
  ...
  for (int batch_idx = 0; batch_idx < batch_size; batch_idx++) {
    ...
    for (int block_id = init_block_num;
      ...
      top_similar_block_[batch_idx].push(
          std::make_pair(block_similar_[batch_idx][block_id],
    ...
    for (; i < init_block_num + pick_block_num + local_block_num; i++) {
      block_table_after_retrieval_[batch_idx][i] = block_table_before_retrieval_
          [batch_idx][(cache_seqlens_[batch_idx] / config_.block_len) -
                      local_block_num + i - init_block_num - pick_block_num];
    }
    if (cache_seqlens_[batch_idx] % config_.block_len != 0) {
      block_table_after_retrieval_[batch_idx][i] =
          block_table_before_retrieval_[batch_idx][(cache_seqlens_[batch_idx] /
                                                    config_.block_len)];
...
void KVCache::retrieval_kvcache_layer_(const uint16_t *q_in_data,
  ...
  if (pick_block_num != -1 &&
    ...
    const auto real_layer_idx =
        (layer_idx - config_.layer_offset) / config_.layer_step;
    if (selected_blocks_num_history_[real_layer_idx] == 0) {
      max_block_num_after_retrieval_ = max_block_num;
      block_table_after_retrieval_.swap(block_table_before_retrieval_);
    } else {

      max_block_num_after_retrieval_ =
          selected_blocks_num_history_[real_layer_idx];

      for (int batch_idx = 0; batch_idx < batch_size; batch_idx++) {
        for (int i = 0; i < max_block_num_after_retrieval_; i++) {
          block_table_after_retrieval_[batch_idx][i] =
              selected_blocks_history_[real_layer_idx][batch_idx][i];
        }

        if (cache_seqlens[batch_idx] % config_.block_len == 1) {
          selected_blocks_num_history_[real_layer_idx] += 1;
          int x = selected_blocks_num_history_[real_layer_idx];
          int last_block_idx =
              block_table_before_retrieval_[batch_idx]
                                           [cache_seqlens[batch_idx] /
                                            config_.block_len];
          selected_blocks_history_[real_layer_idx][batch_idx][x - 1] =
              last_block_idx;
          block_table_after_retrieval_[batch_idx][x - 1] = last_block_idx;
        }
        cache_seqlens_[batch_idx] =
            (cache_seqlens_[batch_idx] % config_.block_len) +
            selected_blocks_num_history_[real_layer_idx] * config_.block_len -
            config_.block_len;
      }
    }
  } else if (pick_block_num != -1) {
    max_block_num_after_retrieval_ = std::min(
        max_block_num, init_block_num + pick_block_num + local_block_num + 1);
    calculate_block_similarity_layer_(
        q_in_data, batch_size, layer_idx, q_len, max_block_num, cache_seqlens,
        init_block_num, local_block_num, pick_block_num, backend);
    select_block_layer_(batch_size, layer_idx, max_block_num, init_block_num,
                        local_block_num, pick_block_num);
  } else {
    selected_blocks_num_history_[(layer_idx - config_.layer_offset) /
                                 config_.layer_step] = 0;
    max_block_num_after_retrieval_ = max_block_num;
...
void KVCache::calculate_sparsity_layer_(const uint16_t *q_in_data,
  ...
  backend->do_work_stealing_job(
      ...
      [&](int task_id) {
        ...
        int head_id =
            (task_id % (config_.kv_head_num * max_block_num)) / max_block_num;
        int block_id = task_id % max_block_num;
        int thread_id = Backend::thread_local_id;
        // If the block is out of the sequence length, skip it.
  ...
  for (int i = 0; i < batch_size; i++) {
    for (int j = 0; j < max_block_num_after_retrieval_; j++) {
      ...
      for (int k = 0; k < config_.q_head_num; k++) {
        attn_sparsity[i * config_.q_head_num + k] += std::exp(
            block_lse_[i][block_idx][k] - attn_lse_[i][k / n_gqa_][k % n_gqa_]);
      }
    }
  }
...
void KVCache::retrieval_kvcache_kvhead_(const uint16_t *q_in_data,
  ...
  if (pick_block_num != -1 &&
          ...
          for (int i = 0; i < config_.kv_head_num; i++) {
            int last_block_idx =
                block_table_before_retrieval_kvhead_[batch_idx]
                                                    [cache_seqlens[batch_idx] /
                                                     config_.block_len][i];
            selected_blocks_history_kvhead_[(layer_idx - config_.layer_offset) /
                                            config_.layer_step][batch_idx]
                                           [x - 1][i] = last_block_idx;
            block_table_after_retrieval_kvhead_[batch_idx][x - 1][i] =
                last_block_idx;
          }
        }
        cache_seqlens_[batch_idx] =
            std::min(cache_seqlens_[batch_idx],
                     (cache_seqlens_[batch_idx] % config_.block_len) +
                         (init_block_num + pick_block_num + local_block_num) *
                             config_.block_len);
      }
    ...
    calculate_block_similarity_kvhead_(
        q_in_data, batch_size, layer_idx, q_len, max_block_num, cache_seqlens,
        init_block_num, local_block_num, pick_block_num, backend);
    select_block_kvhead_(batch_size, layer_idx, max_block_num, init_block_num,
                         local_block_num, pick_block_num);
  } else {
    selected_blocks_num_history_[(layer_idx - config_.layer_offset) /
                                 config_.layer_step] = 0;
      ...
      [&](int task_id) {
        ...
        for (int head_id = 0; head_id < config_.q_head_num; head_id++) {
          for (int i = 0; i < config_.head_dim; i++) {
            ...
            block_similar_kv_head_[batch_id][block_id][head_id / n_gqa_] +=
                qa_i;
          }
        }
      },
  ...
  for (int batch_idx = 0; batch_idx < batch_size; batch_idx++) {
    ...
    for (int head_id = 0; head_id < config_.kv_head_num; head_id++) {
      ...
      if (cache_seqlens_[batch_idx] % config_.block_len != 0) {
        block_table_after_retrieval_kvhead_[batch_idx][i][head_id] =
            block_table_before_retrieval_kvhead_[batch_idx][(
                cache_seqlens_[batch_idx] / config_.block_len)][head_id];
      ...
      for (int j = 0; j < i; j++) {
        selected_blocks_history_kvhead_
            [(layer_idx - config_.layer_offset) / config_.layer_step][batch_idx]
            [j][head_id] =
                block_table_after_retrieval_kvhead_[batch_idx][j][head_id];
      }
    }
...
void KVCache::get_attn_sparsity(const ggml_fp16_t *q_in, float *attn_sparsity,
  ...
  printf("layer %d time of get attn sparsity: %f s\n", layer_idx,
  ...
  if (q_type == GGML_TYPE_F16) {
    ...
    if (rotary_angle != nullptr) {
        ...
        for (int l = 0; l < head_dim / 32; l++) {
          for (int m = 0; m < 32; m++) {
            ...
            float sin_val = GGML_FP16_TO_FP32(
                ((ggml_fp16_t *)rotary_sin)[angle * head_dim + l * 32 + m]);
            float cos_val = GGML_FP16_TO_FP32(
                ((ggml_fp16_t *)rotary_cos)[angle * head_dim + l * 32 + m]);
      ...
      llamafile_sgemm(past_kv_len, bsz, head_dim,
                      ...
                      GGML_TASK_TYPE_COMPUTE, k_type, GGML_TYPE_F16,
                      GGML_TYPE_F32, GGML_PREC_DEFAULT);
    } else {
      bool ok =
          llamafile_sgemm(past_kv_len, bsz, head_dim, (ggml_fp16_t *)k_cache,
    ...
    // so now the q_type is GGML_TYPE_Q8_0
    // all behavior is same and the only difference is the data type
    ...
    if (rotary_angle != nullptr) {
      ...
      for (int k = 0; k < past_kv_len; k++) {
        for (int l = 0; l < head_dim / 32; l++) {
          ...
          quantize_row_q4_0(block_fp32.data(),
      ...
      llamafile_sgemm(past_kv_len, bsz, head_dim / 32,
                      ...
                      (block_q8_0 *)q, head_dim / 32, attn_score, past_kv_len,
                      0, 1, GGML_TASK_TYPE_COMPUTE, k_type, GGML_TYPE_Q8_0,
                      GGML_TYPE_F32, GGML_PREC_DEFAULT);
    } else {
      llamafile_sgemm(past_kv_len, bsz, head_dim / 32, (block_q4_0 *)k_cache,
                      head_dim / 32, (block_q8_0 *)q, head_dim / 32, attn_score,
                      past_kv_len, 0, 1, GGML_TASK_TYPE_COMPUTE, k_type,
