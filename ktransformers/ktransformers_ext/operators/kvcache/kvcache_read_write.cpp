            ...
            for (int l = 0; l < config_.head_dim / 32; l++) {
              ...
              for (int m = 0; m < 32; m++) {
                ...
                block_fp32[m] = GGML_FP16_TO_FP32(
                    k_data_[((0 * config_.kv_head_num + head_id) * seq_len_ +
          ...
          for (int k = 0; k < config_.block_len / 32; k++) {
            for (int l = 0; l < config_.head_dim; l++) {
              ...
              for (int m = 0; m < 32; m++) {
                ...
                block_fp32[m] = GGML_FP16_TO_FP32(
                    v_data_[((0 * config_.kv_head_num + head_id) * seq_len_ +
  ...
  backend->do_work_stealing_job(
      ...
      [&](int task_id) {
        ...
        if (task_id & 1) {
          ...
          for (int k = 0; k < config_.block_len; k++) {
            for (int l = 0; l < config_.head_dim / 32; l++) {
              block_q4_0 block = k_cache_q4[layer_id_][head_id][block_idx]
          ...
          if (config_.kv_type == ggml_type::GGML_TYPE_F16) {
            for (int k = 0; k < config_.block_len; k++) {
              if (block_id * config_.block_len + k >= seq_len)
                break;
              for (int l = 0; l < config_.head_dim; l++) {
                k_data_[batch_id * (max_block_num * config_.block_len *
                                    config_.kv_head_num * config_.head_dim) +
                        block_id * (config_.block_len * config_.kv_head_num *
                                    config_.head_dim) +
                ...
                v_data_[batch_id * (max_block_num * config_.block_len *
                                    config_.kv_head_num * config_.head_dim) +
                        block_id * (config_.block_len * config_.kv_head_num *
                                    config_.head_dim) +
                        k * (config_.kv_head_num * config_.head_dim) +
            ...
            for (int k = 0; k < config_.block_len; k++) {
              if (block_id * config_.block_len + k >= seq_len)
                break;
              for (int l = 0; l < config_.head_dim / 32; l++) {
                block_q8_0 block = k_cache_q8[layer_id_][head_id][block_idx]
                                             [k * config_.head_dim / 32 + l];
        ...
        if (block_r > seq_len && block_l < seq_len + q_len) {
          if (config_.kv_type == ggml_type::GGML_TYPE_F16) {
            for (int k = 0; k < config_.block_len; k++) {
              ...
              for (int l = 0; l < config_.head_dim; l++) {
                k_cache_fp16_
                    [layer_id_][head_id][block_idx][k * config_.head_dim + l] =
                        k_data_[batch_id *
                                ...
                                head_id * config_.head_dim + l];
                v_cache_fp16_
                    [layer_id_][head_id][block_idx][l * config_.block_len + k] =
                        v_data_[batch_id *
                                    (max_block_num * config_.block_len *
                                     config_.kv_head_num * config_.head_dim) +
                                block_id *
      ...
      [&](int task_id) {
        ...
        if (config_.kv_type == ggml_type::GGML_TYPE_F16) {
                         ...
                         [pos_in_block * config_.head_dim + l] =
                             k_data_[batch_id * (q_len * config_.kv_head_num *
                                     ...
                                     q_offset * config_.kv_head_num *
                                         config_.head_dim +
                                     head_id * config_.head_dim + l];
            v_cache_fp16_[layer_id_][head_id][block_idx][l * config_.block_len +
                                                         pos_in_block] =
                v_data_[batch_id *
                            (q_len * config_.kv_head_num * config_.head_dim) +
                        q_offset * config_.kv_head_num * config_.head_dim +
                        head_id * config_.head_dim + l];
          }
        } else if (config_.kv_type == ggml_type::GGML_TYPE_Q4_0) {
          std::vector<float> block_fp32(32);
          ...
          for (int l = 0; l < config_.head_dim / 32; l++) {
            ...
            k_cache_q4[layer_id_][head_id][block_idx]
                      [pos_in_block * config_.head_dim / 32 + l] = block;
          }
          ...
          for (int l = 0; l < config_.head_dim; l++) {
            ...
            quantize_row_q4_0(block_fp32.data(), &block, 32);
            v_cache_q4[layer_id_][head_id][block_idx]
                      [l * config_.block_len / 32 + pos_in_block / 32] = block;
          }
        } else if (config_.kv_type == ggml_type::GGML_TYPE_Q8_0) {
          std::vector<float> block_fp32(32);
          // fill k_cache_
            ...
            for (int l = 0; l < config_.head_dim; l++) {
              ...
              for (int m = 0; m < 32; m++) {
                ...
                v_data_[(head_id * cache_total_len_ +
                         block_idx * config_.block_len + k * 32 + m) *
                            config_.head_dim +
                        l] = GGML_FP32_TO_FP16(block_fp32[m]);
                max_offset =
                    std::max(max_offset, (int)((head_id * cache_total_len_ +
